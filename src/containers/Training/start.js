...
export default class Start extends React.Component {
  ...
  getFlashcards = (uid) => {
    ...
    let query = db.collection('users').doc(uid).collection('cards');
    if (category) {
        query = query.where("category", "==", category);
    }
    query.where("nextDay", "<", new Date())
  ...
  saveResponse = (cardId, value) => {
    const cardRef = db.collection('users').doc(auth.currentUser.uid).collection("cards").doc(cardId);
  ...
  render() {
    return (
        <>
        {
            ...
            : (
              this.state.cards && (this.state.currentCardIndex < this.state.cards.length) ?
            <>
            ...
            <Container>
              <Row>
                <Col>
                  <h1>You're done!</h1>
                  <p>This was it for this training session. Check back tomorrow for more cards!</p>
                </Col>
              </Row>
            </Container>)
        }
